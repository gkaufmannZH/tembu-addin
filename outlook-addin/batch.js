// Tembu Batch-Analyse v20260629g
...
async function loadFolders() {
  try {
    ...
    document.getElementById('folderList').innerHTML =
      `<span class="folder-loading" style="color:#c00">Fehler beim Laden: ${escHtml(e.message)}</span>`;
  }
}
...
function updateStartButton() {
  ...
  document.getElementById('actionHint').textContent =
    anySelected ? `${_folders.filter(f => f.checked).length} Ordner gewählt` : 'Bitte mindestens einen Ordner wählen';
}
...
async function scanFolders(since) {
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    document.getElementById('progressCurrent').textContent =
      `Scanne Ordner ${i + 1}/${selected.length}: ${folder.displayName}…`;
...
function setProgress(done, total) {
  ...
  document.getElementById('progressText').textContent = `${done} / ${total} Kontakte (${pct}%)`;
}
...
async function startBatch() {
  ...
  if (!isLocalProvider(config.provider) && !config.apiKey) {
    alert('Bitte zuerst einen KI-Provider und API-Key konfigurieren (Kontakt in detail.html öffnen → Einstellungen).');
    _running = false;
    return;
  }
  ...
  try {
    ...
    document.getElementById('progressText').textContent = 'Scanne Ordner…';
    const contacts = await scanFolders(since);
    const total = contacts.size;

    if (total === 0) {
      document.getElementById('progressText').textContent = 'Keine externen Kontakte in den gewählten Ordnern gefunden.';
      document.getElementById('progressCurrent').textContent = '';
    ...
    for (const [email, name] of contacts) {
      if (_cancelled) break;
      setProgress(i, total);
      document.getElementById('progressCurrent').textContent = `Analysiere: ${name}`;

      try {
        const result = await analyzeContact(email, name, since, config);
        if (result.skipped) {
          skipped++; setStat('statSkip', skipped);
          addLogItem(name, email, '⏭', 'skip', 'Übersprungen (< 24h)');
        } else {
          done++; setStat('statDone', done);
          const detail = result.emailCount > 0
            ? `${result.emailCount} Mails${result.meetingCount > 0 ? ` · ${result.meetingCount} Meetings` : ''}`
            : 'Keine Mails';
          addLogItem(name, email, '✓', 'ok', detail);
        }
      } catch (e) {
        errors++; setStat('statErr', errors);
        addLogItem(name, email, '✗', 'err', (e.message || 'Fehler').slice(0, 60));
      }
      i++;
    }
    ...
    document.getElementById('progressText').textContent = 'Fehler: ' + (e.message || String(e));
    document.getElementById('progressCurrent').textContent = '';
    _running = false;
...
function finish(done, skipped, errors, total, cancelled) {
  ...
  document.getElementById('doneText').textContent =
    `${done} Analysen gespeichert · ${skipped} übersprungen · ${errors} Fehler` +
    (cancelled ? '\nVom Benutzer abgebrochen.' : ` (${total} Kontakte total)`);
  _running = false;
  document.getElementById('btnStart').disabled = false;
  document.getElementById('btnCancel').classList.add('hidden');
  updateStartButton();
...
function cancelBatch() {
  ...
  document.getElementById('progressCurrent').textContent = 'Wird abgebrochen…';
}
