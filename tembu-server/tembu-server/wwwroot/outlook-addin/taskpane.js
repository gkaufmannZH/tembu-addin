...
const SERVER_URL_KEY = 'tembu_server_url';
...
let _contextContacts = []; // { name, phone, email, label } from current mail/appt
let _rumbleFormExpanded = false;
...
function onServerUrlChange(url) {
...
// ── Ist dieser Name/diese E-Mail der eigene angemeldete Account? ──────────
function isSelf(name, email) {
  if (!_account) return false;
  if (email && _account.username && email.toLowerCase() === _account.username.toLowerCase()) return true;
  if (name && _account.name && name.toLowerCase() === _account.name.toLowerCase()) return true;
  return false;
}
...
function loadOutlookContext() {
  ...
  if (item.itemType === Office.MailboxEnums.ItemType.Message) {
    ...
    const addMailContact = (name, email) => {
      if (!name || isSelf(name, email)) return;
      _contextContacts.push({ name, phone: '', email: email || null, label: 'Mail' });
      _messageParticipantNames.push(name);
    };
    ...
    const addApptContact = (name, email) => {
      if (!name || isSelf(name, email)) return;
      _contextContacts.push({ name, phone: '', email: email || null, label: 'Termin' });
      _appointmentAttendeeNames.push(name);
    };
...
// ── "Rumble erstellen" blendet nur die Felder + den Speichern-Button ein/aus ──
function expandRumbleForm() {
  _rumbleFormExpanded = true;
  document.getElementById('contactPhoneField')?.classList.remove('hidden');
  document.getElementById('rumbleTextField')?.classList.remove('hidden');
  document.getElementById('followUpSection')?.classList.remove('hidden');
  document.getElementById('btnSaveRumble')?.classList.remove('hidden');
  document.getElementById('rumbleText')?.focus();
}

function collapseRumbleForm() {
  _rumbleFormExpanded = false;
  document.getElementById('contactPhoneField')?.classList.add('hidden');
  document.getElementById('rumbleTextField')?.classList.add('hidden');
  document.getElementById('followUpSection')?.classList.add('hidden');
  document.getElementById('btnSaveRumble')?.classList.add('hidden');
}

function toggleRumbleForm() {
  if (_rumbleFormExpanded) collapseRumbleForm(); else expandRumbleForm();
}
...
async function handleSave() {
  ...
  const btn = document.getElementById('btnSaveRumble');
  const btnLabel = btn.querySelector('span');
  btn.disabled = true;
  if (btnLabel) btnLabel.textContent = TI18n.t('taskpane.rumbleSavingButton');
  clearStatus();
  ...
  try {
    ...
    btn.disabled = false;
    if (btnLabel) btnLabel.textContent = TI18n.t('taskpane.rumbleSaveActionButton');
  }
}
...
function selectParticipant(name) {
  ...
  // update email from _contextContacts
  const ctx = _contextContacts.find(c => c.name === name);
  _contactEmail = ctx?.email || null;
...
function wireEvents() {
  ...
  document.getElementById('btnSave').addEventListener('click', toggleRumbleForm);
...
function openBatchDialog() {
  const params = new URLSearchParams({ t: _token || '', _v: '20260701a' });
